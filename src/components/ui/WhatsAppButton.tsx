...
export function WhatsAppButton({
    ...
    message = 'Hello! I would like to inquire Transport services.'
}: WhatsAppButtonProps) {
    const handleClick = () => {
    ...
    return (
        <div
            className="fixed left-6 z-50 group"
            style={{ bottom: 'calc(env(safe-area-inset-bottom, 0px) + 24px)' }}
        >
            <button
                onClick={handleClick}
                className="relative flex items-center justify-center w-14 h-14 bg-[#25D366] hover:bg-[#20BA5A] text-white rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-110 overflow-hidden"
